...
-- Modification for List-based GUI, Impersonate Player & Core Features: [AI Assistant]
...
makeFeatureButton("RESET AVATAR & STATS", Color3.fromRGB(150, 0, 0), function(button)
    ...
    if success then
        -- Stat default akan di-reset di CharacterAdded
    end
...
player.CharacterAdded:Connect(function(char)
    -- Reset stats default setelah LoadCharacter()
...
-- 🔽 Logika Impersonate Player (Side Panel) 🔽
...
local function makePlayerButton(targetPlayer)
    ...
    tpButton.MouseButton1Click:Connect(function()
        ...
        for _, obj in ipairs(char:GetChildren()) do
            if obj:IsA("Accessory") or obj:IsA("Shirt") or obj:IsA("Pants") then pcall(function() obj:Destroy() end) end
        end
